...
if __name__=='__main__':
    ...
    route_sequence.to_csv('%s_route_sequence.csv' % args[0])
    route_sequence = pd.DataFrame(route_sequence,columns=['route_id','trip_id','shape_id','route_short_name','route_long_name','route_desc','direction_id'])
    print 'writing representative trip id correspondence file'
    route_sequence.to_csv('%s_route_to_repr_trip_ids.csv' % args[0])
    ...
    repr_trips.to_csv('%s_repr_trips.csv' % args[0])
    ...
    for i, route in route_sequence.iterrows():
        ...
        shape_writer.record(route['route_id'],route['route_short_name'],route['route_long_name'],route['direction_id'],route['trip_id'])
